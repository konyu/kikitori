...
### 4. テスト整備（TDDリファクタリング）
- [x] pytest 導入
- [x] `AudioBuffer` のスレッドセーフテスト
- [x] `Recorder` の Fake ストリームによる録音テスト
- [x] `Transcriber` の依存注入テスト
- [x] `Injector` のペースト動作テスト
- [x] `HotkeyManager` の状態遷移テスト
- [x] `App` の統合テスト

### 5. 動作確認
- [x] スクリプト実行（初回はモデルダウンロード発生）。
- [x] macOSのシステム設定から、実行しているターミナル/エディタにマイクとアクセシビリティの権限付与。
- [x] ブラウザやエディタの入力欄にフォーカス。
- [x] ホットキー押下で発声、解放でテキストが自動入力・ペーストされることの確認。

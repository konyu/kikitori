...
- **直感的な UI**:
  - メニューバー常駐でいつでもアクセス可能。
  - 録音中は画面下部寄りに「Apple Liquid Glass」デザインのリアルタイム波形オーバーレイ UI（鳥のアイコンと12本の波形バー）を表示。
...
Kikitori はホットキー（キーボードの修飾キー）を押している間だけ音声を録音し、離すと認識結果を自動入力するシンプルなツールです。アプリを起動すると、メニューバーに鳥のアイコン (🐦) が表示されます。

- **録音の開始**: 設定したホットキー（デフォルトは `Fn (🌐)` キー単体）を**押し続けます**。
  - 画面下部寄りに、鳥のアイコンと12本の波形バーが並んだ「透明なグラスUI」が表示され、声の大きさに合わせて波形がリアルタイムに動きます。
...
メニューバーの鳥のアイコンをクリックすると、以下のメニューが表示されます。各設定項目は専用の GUI ウィンドウで直感的に操作可能です。
